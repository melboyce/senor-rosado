...
func Respond(c *slack.Conn, m *slack.Message) {
	// TODO needs to be cached for a minute or so
	var err error
	token := os.Getenv("DARKSKY_TOKEN")
	if token == "" {
		c.ReplyWithErr(m, "missing Darksky token", fmt.Errorf("DARKSKY_TOKEN missing"))
		return
	}

	loc, err := getLocation(strings.Join(m.Args[1:], ","))
	if err != nil {
		c.ReplyWithErr(m, "unable to convert location", err)
		return
	}

	w, err := getWeather(token, loc)
	if err != nil {
		c.ReplyWithErr(m, "unable to get weather", err)
		return
	}
	...
	r := slack.Reply{Text: fmt.Sprintf(report, w.GoogleName, ts, w.Currently.Summary, w.Currently.Temperature, w.Hourly.Summary, w.Daily.Summary)}
	c.ReplyTo(m, r)
}
